...
window.initMap = function () {
  ...
  fetch(kmlUrl)
    ...
    .then((kmlText) => {
      ...
      console.log(
        "Root element:",
        kmlDoc.documentElement.localName,
        "namespaceURI:",
        kmlDoc.documentElement.namespaceURI
      );
      const ns = kmlDoc.documentElement.namespaceURI;
      ...
      if (!coordsEls || coordsEls.length === 0) {
        console.error("No elements found in KML via any retrieval", kmlDoc);
        return;
      }
      ...
      console.log("Selected under", coordEl.parentNode.localName, "with", maxCount, "points");
      const coordText = coordEl.textContent.trim();
      const path = coordText
        .split(/\s+/)
        .map((pair) => {
